  ...
  float accX, accY, accZ, gyroX, gyroY, gyroZ,
        cur_speed, accRoll,      accPitch,     accYaw;            // units degrees (roll and pitch noisy, yaw not possible)
  ...
  float speed_measurement(float aX){
    // this function is triggered to measure the current speed of the driver
    // the metrics are measured according to the driver's shift on the X axis of the imu sensor
    if(aX>-0.5 && aX<0.5){
  ...
  void getIMUReadings(){
    ...
    cur_speed = speed_measurement(accX);
  ...
  void task1(void * parameters){
    for(;;){
      ...
      if (millis() > last + 1000/portTICK_PERIOD_MS ){
        ...
        message += "speed="+String(cur_speed)+";";
